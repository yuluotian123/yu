...
/// <summary>
/// MissionGraph 中代表一个真�?Mission 的节点�?///
/// 节点进入时不会自己完成，而是�?Mission 部署请求交给 MissionGraphRuntime�?/// MissionManager 后续回调任务完成/移除时，runtime 才会�?completion 写回节点�?/// </summary>
public class MissionNode : GraphNodeData, IFlowNode
{
    /// <summary>
    /// �?Mission 需要满足的需求模板�?    /// </summary>
    [JsonInclude]
    ...
    /// <summary>
    /// 根据当前节点配置创建 Mission 原型�?    /// GraphName 参数历史上叫 GraphName，新的调用语义实际传入的�?graphPath�?    /// 生成�?MissionId �?graphPath.nodeId�?    /// </summary>
    ...
    /// <summary>
    /// FlowGraphRuntime 进入节点时调用�?    /// MissionNode 只排队部署请求，等待 Manager drain 后创建真�?Mission�?    /// </summary>
    ...
    /// <summary>
    /// 查询 Mission 是否已经完成�?    /// completion �?MissionGraphRuntime.OnMissionCompleted 或部署失败路径写入，只消费一次�?    /// </summary>
    ...
    public override Control CreateInspectorUI(GraphEditorContext context)
    {
        ...
        root.AddChild(GraphEditorUi.BuildRow("Mode", modeOption));
